...
- The input of the request that triggers a SAML Sudo Mode confirmation is no longer lost over the IdP round trip. The fields Redmine's own Sudo Mode already selects are sealed into one opaque value that the browser keeps in the `sessionStorage` of the tab that visits the IdP, and are offered back on a resume page afterwards. Nothing about the confirmation itself changes: returning from the IdP still only refreshes the Sudo Mode timestamp and never submits anything, the restored form is an ordinary Redmine form with a fresh CSRF token that the user submits explicitly, and the resumed request passes Redmine's own Sudo Mode check again. A repeated SAML callback or a reloaded page therefore cannot perform the original change. The value is encrypted and authenticated with `ActiveSupport::MessageEncryptor` from a key derived from `secret_key_base`, is bound to the user and the login session that created it, expires after 15 minutes, and gets a storage key of its own so that two tabs cannot overwrite each other's input. The SAML transaction remains single-flight per Redmine login session: a later tab neither cancels the pending transaction nor starts another `AuthnRequest`, and instead keeps its own continuation until the first tab finishes; a failure or expiry lets it start a new confirmation. Which request gets the transaction is decided by an insert on the unique index of Redmine's existing `tokens` table rather than by a check against the session cookie, so it holds for genuinely simultaneous tabs too, on every database Redmine supports. The scope is one Redmine login session rather than one user, so a second browser or device of the same user confirms independently and neither cancels the other; the continuation binding is derived from the same login session identity instead of being written to the session, so concurrent tabs agree on it and the feature stores nothing in the session at all. A SAML callback that does not win its transaction leaves the session completely untouched, so it can never roll back the Sudo timestamp, SAML identity or Single Logout context of the request that did win. No new setting, no migration, no server-side storage and no change to normal SAML login, logout or Single Logout. `GET` requests, the modal shown for remote forms, and raw multipart uploads keep the 1.2.0 behaviour of not being resumed.
